  ...
  const handleSubmitApplication = async () => {
    try {
      ...
      if (wasGTACB) {
        ...
        if (applicationData.gtaCourseHistory.length!=0){
      ...
      const response = await axios.post('http://localhost:8080/gta/application', {
        ...applicationData,
        files: formData // Include files as a separate field
      });
